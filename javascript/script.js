...
//index.html・category/index.html・product/index.html・search/index.html・data.jsonのURL
const url_1 = 'http://127.0.0.1:5500/index.html';
const url_2 = 'http://127.0.0.1:5500/category/index.html?';
const url_3 = 'http://127.0.0.1:5500/product/index.html?';
const url_search = 'http://127.0.0.1:5500/search/index.html?';
    ...
    $.ajax({ url: url_json, datatype: 'json' })
        .done(function (data) {
            data.forEach(function (item, index) {
                ...
                //index.htmlで.category[0]をクリックするとcategory/index.htmlへ
                //category/index.htmlで.category[0]をクリックするとproduct/index.htmlへ
                ...
                //ここから現在地がcategory/index.htmlの場合-------------------------------------------------------------------------------------------------------------------
                if (current == url_2 + 'category=' + item.category[0]) {
                    ...
                    //product/index.html遷移時に表示するデータを保存
                ...
                //ここまで現在地がcategory/index.htmlの場合-------------------------------------------------------------------------------------------------------------------

                //productが含まれているデータのみを取得
                if (item.product != null) {

                    //product/index.htmlのproduct情報を特定する
                    if (current.indexOf(url_3 + 'category=' + item.category[0]) != -1) {
                        ...
                        for (let page_num = 1; page_num <= page_limit; page_num++) {
                            //ここから現在地がproduct/index.htmlの場合-------------------------------------------------------------------------------------------------------------------
                            if (current == url_3 + 'category=' + item.category[0] + '&page=' + page_num) {
                                //パンくずリストにcategory/index.htmlで保存したデータを追加
                                let breadcrumb_2html = document.querySelector('#breadcrumb_2html')
